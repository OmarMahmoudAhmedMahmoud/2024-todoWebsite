const webStatus = {
  ...
  eleIndex: localStorage.getItem("index"),
};

function localSDataFirstTime() {
  ...
  if (localStorage.getItem("firstTime") == null) {
    ...
    for (let i = 0; i < index; i++) {
      if (localStorage.getItem(i) != null) {
        localStorage.setItem(`${i}title`,localStorage.getItem(i))
        localStorage.setItem(`${i}link`,'')
      }
...
function addElement(title, link) {
  webStatus.eleIndex++;
  localStorage.setItem("index", webStatus.eleIndex);
  localStorage.setItem(`${webStatus.eleIndex}title`, title);
  localStorage.setItem(`${webStatus.eleIndex}link`, link);
  new TodoBoxes(webStatus.eleIndex);
}

function removeElement(id) {
  localStorage.removeItem(`${id}title`);
  localStorage.removeItem(`${id}link`);
}

function eventesRender(id) {
  let mainBox = document.getElementById("mainTodoBox");
  let box = document.getElementById(`todoBox${id}`);

  let deletBtn = document.getElementById(`todoDelet${id}`);
  let editBtn = document.getElementById(`todoEdite${id}`);
  let saveBtn = document.getElementById(`todoSave${id}`);
  ...
  function hover(bool) {
    ...
    if (bool) {
      box.addEventListener("mouseenter", block);

      box.addEventListener("mouseleave", none);

      return;
    }

    box.addEventListener("mouseenter", none);
  }
  ...
  let linkBar = document.getElementById(`todoInput${id}`);
  let titleBar = document.getElementById(`todoP${id}`);
  function edit() {
  ...
  function save() {
    saveBtn.addEventListener("click", function () {
      ...
      hover(true);
      openLink(id, linkBar.value);
      titleBar.setAttribute("contenteditable", "false");

      localStorage.setItem(`${id}title`, titleBar.textContent);
      localStorage.setItem(`${id}link`, linkBar.value);
    });
  }
  ...
  function openLink(id, link) {
    ...
    if (link.trim() == "") {
      box.setAttribute("data-link", "false");
      return;
    }
    box.setAttribute("data-link", "link");
    box.addEventListener("dblclick", box.addEventListener("dblclick", linkGo));
  }
...
const input = {
  bool: false,
  btn: document.getElementById("close-open-btn"),
  box: document.getElementById("mainInputs"),
  addBtn: document.getElementById("addBtn"),
  add: function () {
    let inputTitle = document.getElementById("inputs-title");
    let inputLink = document.getElementById("inputs-link");
    ...
    if (titleValue.trim().length == 0) {
      inputTitle.style.outline = "3px #009179 solid";
      return;
    }

    inputTitle.style.border = "none";

    addElement(titleValue, linkValue);
    let box = document.getElementById("mainTodoBox");
    let lastElePos = box.lastChild.offsetTop;
    box.scrollBy(0,lastElePos)
  ...
  openClose: function () {
    ...
    let wi;
    if (webStatus.with() > 700) {
      wi = "20vw";
    } else {
      wi = "60vw";
    }
    if (this.bool) {
      input.btn.style.transform = "rotate(45deg)";
      input.box.style.paddingRight = wi;
    } else {
      input.btn.style.transform = "rotate(0deg)";
      input.box.style.paddingRight = "0vw";
      let inputTitle = document.getElementById("inputs-title");
  ...
  render: function () {
    input.btn.addEventListener("click", input.openClose);
    input.addBtn.addEventListener("click", input.add);
    ...
    inputLink.addEventListener("keydown",function (event) {
      if (event.keyCode != 13) {
        return;
      }
...
function render() {
  function todoEles() {
    let time = 50;
    for (let i = 1; i <= webStatus.eleIndex; i++) {
